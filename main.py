...
def domain_handler(message):
    ...
    if now - last < SCAN_COOLDOWN:
        ...
        bot.reply_to(
            message,
            f"⏳ {remaining}s পরে আবার scan করুন.",
            reply_markup=persistent_menu(),
        )
        return
    ...
    # IMPORTANT:
    # Only the actual run_scan() phase can produce LIVE SCAN FAILED.
    # Telegram edit/send errors are delivery errors, never scan errors.
    try:
        ...
        try:
            ...
            print(f"Failure message edit failed: {edit_error}")
            try:
                bot.send_message(
                    message.chat.id,
                    f"❌ Scan failed:\n`{str(e)[:3500]}`",
                ...
                print(f"Failure fallback message failed: {fallback_error}")
        return

    ACTIVE_SCANS.pop(message.from_user.id, None)

    # Scan is successful from this point onward.
    notify_admin(
        "🟢 *LIVE SCAN COMPLETED*\n\n"
        f"👤 User: `{user_label(message.from_user.id)}`\n"
        f"🌐 Website: `{domain}`\n"
    ...
    # Report delivery is deliberately separate from scan execution.
    # Fast Scan  -> report stays in chat.
    # Deep Scan  -> full report is always sent as a document.
    try:
        if mode == "fast":
            # Keep Fast Scan in chat. Telegram allows max ~4096 chars per
            # message, so send the report in safe chunks when necessary.
            report_html = html.escape(report).replace(
                "Developed by PH Hamid",
                '<a href="https://t.me/PHhamid">Developed by PH Hamid</a>',
            )

            chunks = []
            remaining = report_html
            while remaining:
            ...
            # First chunk replaces the progress message. If editing fails,
            # simply send it as a new message—never mark the scan failed.
            first = chunks[0] if chunks else "✅ FAST Scan completed."
            try:
                bot.edit_message_text(
                    first,
                    message.chat.id,
                ...
                print(f"Fast report edit failed (scan successful): {edit_error}")
                try:
                    bot.send_message(
                        message.chat.id,
                        first,
                    ...
                    print(f"Fast report send failed (scan successful): {send_error}")

            # Remaining Fast Scan chunks stay in the chat.
            for chunk in chunks[1:]:
                try:
                    ...
                    print(f"Fast report chunk failed (scan successful): {chunk_error}")

        else:
            # Deep Scan ALWAYS produces a document, regardless of report size.
            # This restores the original intended behavior.
            completion_html = (
                f"✅ <b>DEEP Scan completed</b> — "
                f"<code>{html.escape(domain)}</code>\n"
                f'👤 <a href="https://t.me/PHhamid">Developed by PH Hamid</a>'
            )
            ...
            try:
                ...
                # The progress message may already have changed or Telegram
                # may reject the edit. This is NOT a scan failure.
                print(f"Deep completion edit failed (scan successful): {edit_error}")
                try:
                    bot.send_message(
                        message.chat.id,
                        completion_html,
                    ...
                    print(f"Deep completion fallback failed: {completion_error}")

            try:
                document = io.BytesIO(report.encode("utf-8"))
                document.name = f"{domain}_deep_report.txt"
                bot.send_document(
                    message.chat.id,
                    document,
                    caption="📄 Full DEEP SCAN report",
                    reply_markup=persistent_menu(),
                )
            except Exception as doc_error:
                # Document delivery failed, but the scan itself succeeded.
                # Do not send LIVE SCAN FAILED here.
                print(f"Deep report document send failed (scan successful): {doc_error}")
                try:
                    bot.send_message(
                        message.chat.id,
                        "⚠️ Scan completed, but the report document could not be delivered.",
                        reply_markup=persistent_menu(),
                    )
                except Exception as final_notice_error:
                    print(f"Deep delivery notice failed: {final_notice_error}")

    except Exception as delivery_error:
        # Last-resort guard: ANY Telegram/report-delivery exception must never
        # be converted into a scan failure notification.
        print(f"Unexpected report delivery error (scan successful): {delivery_error}")
